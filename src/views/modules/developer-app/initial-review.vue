  ...
  <div class="develop">
    <el-form :inline="true" :model="dataForm">
      ...
      <el-form-item>
        <el-select v-model="dataForm.status" clearable :placeholder="$t('developer_app.status')">
          <el-option v-for="item in $t('developer_app.initial_examine.status_option')" :key="item.value" :value="item.value"
    ...
    <el-table :data="dataList" v-loading="dataListLoading" border style="width: 100%;">
      ...
      <el-table-column
        ...
        align="center"
        min-width="100"
        :label="$t('developer_app.examiner')">
      </el-table-column>
      <el-table-column
        header-align="center"
        align="center"
    ...
    <app-version-detail v-if="appVersionDetailVisible" ref="appVersionDetail" @close="appVersionDetailVisible = false" @refreshDataList='getDataList'></app-version-detail>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      getInitialExamine(params).then(data => {
