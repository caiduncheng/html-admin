...
import {newOperatorAccountTrend} from '../../../../api/statistic'

export default {
  name: 'newUserCountChart',
  mixins: [resize],
  data () {
    return {
      series: [],
  ...
  methods: {
    initData (dateUnit, timeRange) {
      ...
      newOperatorAccountTrend(params).then((res) => {
        var isEmpty = true
        ...
        series: [
          {
            ...
            // name: this.$t('data_center.new_user_count'),
            type: 'line',
            data: data.yaxisData
          }
        ]
