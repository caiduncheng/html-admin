...
import {cmdFlyDeskTrend} from '../../../../api/statistic'

export default {
  name: 'flydeskCommandNewUsageTimesChart',
  mixins: [resize],
  data () {
    return {
      series: [],
  ...
  methods: {
    initData (dateUnit, timeRange) {
      ...
      cmdFlyDeskTrend(params).then((res) => {
        var isEmpty = true
    ...
    initChart (data) {
      ...
      this.option = {
        ...
        legend: {
          data: ['New usage times of FlyDesk commands']
          // data: [this.$t('data_center.flydesk_cmd_new_usage_times')]
        },
        toolbox: {
          show: true,
          feature: {
        ...
        series: [
          {
            name: 'New usage times of FlyDesk commands',
            // name: this.$t('data_center.flydesk_cmd_new_usage_times'),
            type: 'line',
            data: data.yaxisData
          }
        ]
