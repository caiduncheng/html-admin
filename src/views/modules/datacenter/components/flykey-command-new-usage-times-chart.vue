...
import {cmdFlyKeyTrend} from '../../../../api/statistic'

export default {
  name: 'flykeyCommandNewUsageTimesChart',
  mixins: [resize],
  data () {
    return {
      series: [],
  ...
  methods: {
    initData (dateUnit, timeRange) {
      ...
      cmdFlyKeyTrend(params).then((res) => {
        var isEmpty = true
    ...
    initChart (data) {
      ...
      this.option = {
        ...
        legend: {
          data: ['New usage times of FlyKey commands']
          // data: [this.$t('data_center.flykey_cmd_new_usage_times')]
        },
        toolbox: {
          show: true,
          feature: {
        ...
        series: [
          {
            name: 'New usage times of FlyKey commands',
            // name: this.$t('data_center.flykey_cmd_new_usage_times'),
            type: 'line',
            data: data.yaxisData
          }
        ]
