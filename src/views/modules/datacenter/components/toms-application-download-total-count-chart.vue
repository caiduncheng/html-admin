...
import {statisticApplicationDownloadCount} from '../../../../api/statistic'

export default {
  name: 'tomsApplicationDownloadTotalCountChart',
  mixins: [resize],
  data () {
    return {
      series: [],
  ...
  methods: {
    initData (dateUnit, timeRange) {
      ...
      statisticApplicationDownloadCount(params).then((res) => {
        var isEmpty = true
    ...
    initChart (data) {
      ...
      this.option = {
        ...
        legend: {
          // data: [this.$t('data_center.toms_app_download_total_count')]
          data: ['Total number of TOMS application downloads']
        },
        toolbox: {
          show: true,
          feature: {
        ...
        series: [
          {
            name: 'Total number of TOMS application downloads',
            // name: this.$t('data_center.toms_app_download_total_count'),
            type: 'bar',
            data: data.yaxisData
          }
        ]
