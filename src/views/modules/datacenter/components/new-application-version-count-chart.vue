...
import {applicationVersionTrend} from '../../../../api/statistic'

export default {
  name: 'newApplicationVersionCountChart',
  mixins: [resize],
  data () {
    return {
      series: [],
  ...
  methods: {
    initData (dateUnit, timeRange) {
      ...
      applicationVersionTrend(params).then((res) => {
        var isEmpty = true
    ...
    initChart (data) {
      ...
      this.option = {
        ...
        legend: {
          data: ['Number of new TOMS application versions']
          // data: [this.$t('data_center.new_app_version_count')]
        },
        toolbox: {
          show: true,
          feature: {
        ...
        series: [
          {
            name: 'Number of new TOMS application versions',
            // name: this.$t('data_center.new_app_version_count'),
            type: 'line',
            data: data.yaxisData
          }
        ]
