...
import {statisticDevice} from '../../../../api/statistic'

export default {
  name: 'newTerminalCountChart',
  mixins: [resize],
  data () {
    return {
      series: [],
  ...
  methods: {
    initData (dateUnit, timeRange) {
      statisticDevice(dateUnit, timeRange).then((res) => {
        var isEmpty = true
    ...
    initChart (data) {
      ...
      this.option = {
        ...
        legend: {
          data: ['Number of new terminals']
          // data: [this.$t('data_center.new_terminal_count')]
        },
        toolbox: {
          show: true,
          feature: {
        ...
        series: [
          {
            // name: this.$t('data_center.new_terminal_count'),
            name: 'Number of new terminals',
            type: 'line',
            data: data.yaxisData
          }
        ]
