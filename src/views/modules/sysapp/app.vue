<template>
  <div>
      <div v-if="showUpload">
        <upload-page @backHandle="backHandle" appTypeCode='03'/>
      </div>
    <div v-else class="mod-menu app-store">
      <el-row>
        <el-button v-if="isAuth('10400400.MANAGE')" type="success" @click="uploadHandle" style="margin-top: 10px" icon="el-icon-upload">{{ $t('sysapp.upload') }}</el-button>
      </el-row>
      ...
      <upload v-if="uploadVisible" ref="upload" @refreshDataList="getAppDataList" @close="uploadVisible = false"></upload>
    </div>
  </div>
</template>

<script>
import {getSysAppList, getAppInfoPage, downloadApp, deleteApp, sysProductionRelease} from '../../../api/sysapp'
